...
- **Tests only cover the pure logic modules.** `tests/test_match_quality.py` unit-tests `get_match_quality` and `tests/test_price_extraction.py` unit-tests `extract_price`, both in isolation from Selenium; the browser-driving code (`search_amazon`, `search_flipkart`, `search_google`, `process_products`) has no automated coverage since it requires a real Chrome session.
